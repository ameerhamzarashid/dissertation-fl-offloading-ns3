...
for seed in "${SEEDS[@]}"; do
  ...
  # collect logs
  OUT_RAW="${ROOT}/results/exp3/seed_${seed}/raw_ns3"
  mkdir -p "${OUT_RAW}"
  cp -f "${ROOT}/data/raw_logs/"*.csv "${OUT_RAW}/" || true

  # ingest to per-seed results
  python -m python_fl.scripts.run_exp3 \
    --config "${ROOT}/configs/exp3_scalability.yaml" \
    --ns3-log-dir "${OUT_RAW}" \
    --window-s "${WINDOW_S}"
done

echo "Multi-seed Exp3 runs complete. Results under results/exp3/seed_*/"
